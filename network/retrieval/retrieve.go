		...
		for _, lbPeer := range bin.LBPeers {
			...
			if myPo < depth { //  chunk is NOT within the neighbourhood
				if bin.ProximityOrder <= myPo { // always choose a peer strictly closer to chunk than us
					r.logger.Info("AAAAA", "req.Addr", req.Addr, "Peer", id)
					//return false
				}
				...
				if bin.ProximityOrder < depth { // do not select peer outside the neighbourhood. But allows peers further from the chunk than us
					r.logger.Info("BBBBBB", "req.Addr", req.Addr, "Peer", id)
					//return false
					...
					r.logger.Info("CCCCCCC", "req.Addr", req.Addr, "Peer", id)
					//return false
				}
			}
			...
			if bin.ProximityOrder < myPo && depth > bin.ProximityOrder {
				...
				r.logger.Info("DDDDDD", "req.Addr", req.Addr, "Peer", id)
				//return false
			}
			...
			if depth <= myPo && depth > bin.ProximityOrder {
				...
				r.logger.Info("EEEEEEEEE", "req.Addr", req.Addr, "Peer", id)
				//return false
			}
...
FINDPEER:
	...
	if protoPeer == nil {
		r.logger.Info("findPeer returned a peer to skip", "peer", sp.String(), "retry", retries, "ref", req.Addr)
